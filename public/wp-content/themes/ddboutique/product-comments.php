...
if ( post_password_required() ) {
	_e( 'This post is password protected. Enter the password to view reviews.',TEMPLATE_DOMAIN);
	return;
}
...
<?php if ( have_comments() ) : ?>
    <h3 id="comments"><?php comments_number('No Reviews', 'One Review', '% Reviews' );?></h3>
...
<?php else : // this is displayed if there are no reviews so far ?>
        ...
        <p id="comment-prompt"><?php _e('There are no reviews yet, add one below.',TEMPLATE_DOMAIN); ?></p>
    <?php else : ?>
        <p id="comment-prompt"><?php _e('Reviews are closed.',TEMPLATE_DOMAIN); ?></p>
    <?php */ endif; ?>
...
<?php if ('open' == $post->comment_status) : ?>
     ...
     <div id="respond">   
        <h2><?php comment_form_title( 'Leave a Review', 'Leave a Reply to %s' ); ?></h2>
    
        <?php if ( get_option('comment_registration') && !$user_ID ) {
        $url_open = '<a href="' . home_url() . '/wp-login.php?redirect_to=' .get_permalink() . '">';
        $url_close = '</a>';
        ?>
        <p><?php printf(_x('You must be %1$s logged in %2$s to post a review.','Variables 1 and 2 are the opening and closing tags for a link to the login page',TEMPLATE_DOMAIN),$url_open,$url_close); ?></p>
        ...
        <form action="<?php echo get_option('siteurl'); ?>/wp-comments-post.php" method="post" id="commentform">
            ...
            <li>
            	<label for="comment"><?php _e('Review',TEMPLATE_DOMAIN); ?></label>
            	<textarea name="comment" id="comment" rows="7" tabindex="4"></textarea></li>
    
            <li><input class="submit-btn" name="submit" type="submit" id="submit" tabindex="5" value="<?php _e('Submit Review',TEMPLATE_DOMAIN); ?>" />
